...
function timelineMovementCount(period, pattern) {
  return state.leads.filter(lead => (lead.timeline || []).some(item => item.at && inPeriod(item.at, period) && pattern.test(item.label || ""))).length;
}
